...
def init(request):
	...
	with conn.cursor() as curs:
		try:
			curs.execute("""CREATE TABLE IF NOT EXISTS ex05_movies(
...
def display(request):
	...
	return render(request, "ex05/display.html", {"movielist": movielist})

def remove(request):
	if request.method == "POST":
		r = Movies.objects.filter(episode_nb=request.POST["movies"])
		r.delete()
		HttpResponseRedirect("/ex05/remove")
	m = Movies.objects.all()
	movielist = []
	for movie in m:
		movielist.append((movie.title, movie.episode_nb, movie.opening_crawl, movie.director, movie.producer, movie.release_date))
	if len(movielist) == 0:
		return HttpResponse("No data available")
	return render(request, "ex05/remove.html", {"movielist": movielist})